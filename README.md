...
| **Document Parsing** | python-docx (.docx), pypdf (.pdf), plain text (.md, .txt) |
...
```bash
git clone https://github.com/dxcax/Microsoft_Rag_Emir.git
cd Microsoft_Rag_Emir
```
...
├── foundry_local_client.py  # Foundry Local SDK client
├── sqlite_db.py             # SQLite vector database manager
├── requirements.txt         # Python dependencies
├── README.md                # Project documentation
│
├── data/
│   ├── documents/           # Place your .docx / .pdf / .md / .txt files here
...
- Documents in `data/documents/` are read (`.docx`, `.pdf`, `.md`, `.txt`)
...
- **Hybrid search** (Cosine Similarity + Keyword boost) finds the top-K most relevant chunks from SQLite
- Confidence scores are calibrated to an intuitive 80-98% display range
...
1. Drop any `.docx`, `.pdf`, `.md`, or `.txt` file into `data/documents/`
...
Once the server is running, open [http://127.0.0.1:8000](http://127.0.0.1:8000) and:

- Drag & drop a `.pdf`, `.docx`, `.md` or `.txt` file into the upload area
- Type a question in the chat box and press Enter
- Check the source citation cards below each answer to verify retrieval accuracy
